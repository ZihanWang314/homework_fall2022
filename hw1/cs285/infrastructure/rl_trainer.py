...
class RL_Trainer(object):
    ...
    def collect_training_trajectories(
        ...
        paths, envsteps_this_batch = utils.sample_trajectories(self.env, collect_policy, batch_size, self.params['ep_len'], self.log_video)
    ...
    def train_agent(self):
        ...
        for train_step in range(self.params['num_agent_train_steps_per_iter']):
            ...
            ob_batch, ac_batch, re_batch, next_ob_batch, terminal_batch = map(
                lambda x:torch.tensor(x).to(ptu.device), sample_out
                )
    ...
    def do_relabel_with_expert(self, expert_policy, paths):
        ...
        for i in range(len(paths)):
            observation = torch.tensor(paths[i]['observation']).to(ptu.device)
            paths[i]['action'] = expert_policy(observation).detach().cpu().numpy()
