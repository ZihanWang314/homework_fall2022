...
class MLPPolicyAWAC(MLPPolicy):
    ...
    def update(self, observations, actions, adv_n=None):
        ...
        weights = torch.softmax(adv_n / self.lambda_awac, dim=0)
        policy_logpp = self(observations).log_prob(actions)
        actor_loss = (-policy_logpp * len(weights) * weights.detach()).mean()
