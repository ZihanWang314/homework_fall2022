...
class AWACAgent(DQNAgent):
    ...
    def get_qvals(self, critic, obs, action):
        ...
        qa_values = ptu.from_numpy(critic.qa_values(obs))
        q_values = torch.gather(qa_values, 1, action.long().unsqueeze(1)).squeeze(1)
        return q_values
    ...
    def train(self, ob_no, ac_na, re_n, next_ob_no, terminal_n):
        ...
        if (self.t > self.learning_starts
            ...
            advantage = self.estimate_advantage(ob_no, ac_na, re_n, next_ob_no, terminal_n)#def compute_loss_pi
